...
func TestParseAndMarshalPSISection(t *testing.T) {
	for _, want := range []*PSISection{psi.Sections[2], psi.Sections[3]} {
		var buf bytes.Buffer
		writer := astikit.NewBitsWriter(astikit.BitsWriterOptions{Writer: &buf})
		_, err := writePSISection(writer, want)
		assert.NoError(t, err)

		got, err := ParsePSISection(buf.Bytes())
		assert.NoError(t, err)
		assert.Equal(t, want, got)

		encoded, err := MarshalPSISection(got)
		assert.NoError(t, err)
		assert.Equal(t, buf.Bytes(), encoded)
	}
}

func TestPacketizePSISection(t *testing.T) {
	adaptationField, err := NewStuffingAdaptationField(164)
	assert.NoError(t, err)
	source := &Packet{
		Header: PacketHeader{
	...
	packet, err := PacketizePSISection(source, psi.Sections[3])
	assert.NoError(t, err)
	assert.Len(t, source.Payload, 20)
	assert.Equal(t, byte(0), packet.Payload[0])
	assert.Equal(t, MpegTsPacketSize, 4+AdaptationFieldSizeWithoutStuffing(packet.AdaptationField)+packet.AdaptationField.StuffingLength+len(packet.Payload))

	section, err := ParsePSISection(packet.Payload[1:])
	assert.NoError(t, err)
	assert.Equal(t, psi.Sections[3], section)
}
