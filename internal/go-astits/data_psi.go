...
func ParsePSISection(data []byte) (*PSISection, error) {
	...
	section, stop, err := parsePSISection(i)
	if err != nil {
		return nil, fmt.Errorf("astits: parsing PSI section: %w", err)
	}
	if stop {
		return nil, fmt.Errorf("astits: PSI section is stuffing or an unsupported table")
	}
	if i.HasBytesLeft() {
		return nil, fmt.Errorf("astits: PSI section has %d trailing bytes", i.Len()-i.Offset())
	}
	return section, nil
}

func MarshalPSISection(section *PSISection) ([]byte, error) {
	if section == nil {
		return nil, fmt.Errorf("astits: cannot marshal a nil PSI section")
	}

	var buf bytes.Buffer
	w := astikit.NewBitsWriter(astikit.BitsWriterOptions{Writer: &buf})
	if _, err := writePSISection(w, section); err != nil {
		return nil, fmt.Errorf("astits: marshaling PSI section: %w", err)
	}
	return buf.Bytes(), nil
}

func PacketizePSISection(packet *Packet, section *PSISection) (*Packet, error) {
	encoded, err := MarshalPSISection(section)
	if err != nil {
		return nil, err
	}

	output := ClonePacket(packet)
	required := len(encoded) + 1
	if required > len(output.Payload) {
		adaptationSize := MpegTsPacketSize - 4 - len(output.Payload)
		targetSize := adaptationSize - (required - len(output.Payload))
		minimumSize := AdaptationFieldSizeWithoutStuffing(output.AdaptationField)
		if !HasAdaptationFieldData(output.AdaptationField) {
			minimumSize = 0
		}
		if targetSize < minimumSize {
			return nil, fmt.Errorf("astits: PSI section does not fit one TS packet")
		}
		if targetSize == 0 {
			output.Header.HasAdaptationField = false
			output.AdaptationField = nil
		} else {
			output.Header.HasAdaptationField = true
			output.AdaptationField, err = AdaptationFieldWithSize(
				output.AdaptationField,
				targetSize,
			)
			if err != nil {
				return nil, err
		...
		output.Payload = make([]byte, required)
	} else {
		for i := range output.Payload {
			output.Payload[i] = 0xFF
		}
	}
	output.Header.HasPayload = true
	output.Header.PayloadUnitStartIndicator = true
	output.Payload[0] = 0
	copy(output.Payload[1:], encoded)
	return output, nil
}
