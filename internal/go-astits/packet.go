...
func ClonePacket(packet *Packet) *Packet {
	...
	cloned.AdaptationField = CloneAdaptationFieldWithoutStuffing(packet.AdaptationField)
	cloned.AdaptationField.Length = packet.AdaptationField.Length
	cloned.AdaptationField.StuffingLength = packet.AdaptationField.StuffingLength
	cloned.AdaptationField.IsOneByteStuffing = packet.AdaptationField.IsOneByteStuffing
	cloned.AdaptationField.TransportPrivateDataLength = packet.AdaptationField.TransportPrivateDataLength
	if packet.AdaptationField.AdaptationExtensionField != nil {
		cloned.AdaptationField.AdaptationExtensionField.Length =
			packet.AdaptationField.AdaptationExtensionField.Length
	}
	return &cloned
}

const maxPacketAdaptationFieldSize = MpegTsPacketSize - 4

// HasAdaptationFieldData reports whether field contains metadata other than
// packet stuffing.
func HasAdaptationFieldData(field *PacketAdaptationField) bool {
	return field != nil && (field.DiscontinuityIndicator ||
		field.RandomAccessIndicator ||
		field.ElementaryStreamPriorityIndicator ||
		field.HasPCR ||
		field.HasOPCR ||
		field.HasSplicingCountdown ||
		field.HasTransportPrivateData ||
		field.HasAdaptationExtensionField)
}

// CloneAdaptationFieldWithoutStuffing deep-copies semantic adaptation metadata
// and clears parsed lengths and output stuffing.
func CloneAdaptationFieldWithoutStuffing(
	...
	cloned := *field
	cloned.Length = 0
	cloned.StuffingLength = 0
	cloned.IsOneByteStuffing = false
	if field.PCR != nil {
		pcr := *field.PCR
		cloned.PCR = &pcr
	}
	...
	cloned.TransportPrivateData = append(
		[]byte(nil),
		field.TransportPrivateData...,
	)
	cloned.TransportPrivateDataLength = len(cloned.TransportPrivateData)
	if field.AdaptationExtensionField != nil {
		extension := *field.AdaptationExtensionField
		extension.Length = 0
		if field.AdaptationExtensionField.DTSNextAccessUnit != nil {
			dts := *field.AdaptationExtensionField.DTSNextAccessUnit
			extension.DTSNextAccessUnit = &dts
		}
		cloned.AdaptationExtensionField = &extension
	}
	return &cloned
}

// AdaptationFieldSizeWithoutStuffing returns the serialized size, including
// the adaptation_field_length byte, after output stuffing is removed.
func AdaptationFieldSizeWithoutStuffing(
	field *PacketAdaptationField,
) int {
	if field == nil {
		return 0
	}
	normalized := CloneAdaptationFieldWithoutStuffing(field)
	return 1 + int(calcPacketAdaptationFieldLength(normalized))
}

// AdaptationFieldWithSize returns a deep copy padded to totalSize serialized
// bytes.
func AdaptationFieldWithSize(
	field *PacketAdaptationField,
	totalSize int,
) (*PacketAdaptationField, error) {
	if field == nil {
		return NewStuffingAdaptationField(totalSize)
	}

	cloned := CloneAdaptationFieldWithoutStuffing(field)
	minimum := AdaptationFieldSizeWithoutStuffing(cloned)
	if totalSize < minimum {
		return nil, fmt.Errorf(
			"astits: adaptation field requires %d bytes, target size is %d",
			minimum,
			totalSize,
		)
	}
	if totalSize > maxPacketAdaptationFieldSize {
		return nil, fmt.Errorf(
			"astits: adaptation field target size %d exceeds maximum %d",
			totalSize,
			maxPacketAdaptationFieldSize,
		)
	}
	cloned.StuffingLength = totalSize - minimum
	return cloned, nil
}

// NewStuffingAdaptationField returns an adaptation field that occupies exactly
// totalSize serialized bytes.
func NewStuffingAdaptationField(
	totalSize int,
) (*PacketAdaptationField, error) {
	if totalSize < 1 || totalSize > maxPacketAdaptationFieldSize {
		return nil, fmt.Errorf(
			"astits: stuffing adaptation field size %d is outside [1,%d]",
			totalSize,
			maxPacketAdaptationFieldSize,
		)
	}
	if totalSize == 1 {
		return &PacketAdaptationField{IsOneByteStuffing: true}, nil
	}
	return &PacketAdaptationField{StuffingLength: totalSize - 2}, nil
}
...
func parsePacketAdaptationField(i *astikit.BytesIterator) (a *PacketAdaptationField, err error) {
	...
	// mpegenc's custom fix for zero-length adaptation fields
	a.IsOneByteStuffing = a.Length == 0
